...
public class Resource_adapter extends ArrayAdapter<Resource> {

    double lat1, lon1;

    public Resource_adapter(Context context, int resource, List<Resource> objects, double lat1, double lon1) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        double dist = getDistance(lat1, lon1, resource.getLat(), resource.getLon(),'K');
        distTextView.setText((int)dist+"m away from you");
        messageTextView.setText((resource.getStatus()).toUpperCase()+": "+resource.getRes_name());
        descTextView.setText(resource.getQuantity()+" for approximately "+resource.getSupply_for()+" people.");
